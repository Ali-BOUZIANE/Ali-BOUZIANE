# Salut 👋, je suis Ali Bouziane  
### Ingénieur en Génie Informatique | Cybersécurité

<div align="center">
  <a href="https://github.com/AliBouziane">
    <img src="https://img.shields.io/github/followers/AliBouziane?label=Followers&style=social" alt="Followers GitHub">
  </a>
  <a href="mailto:bouziane_ali@upf.ac.ma">
    <img src="https://img.shields.io/badge/Email-bouziane__ali@upf.ac.ma-red?style=flat-square&logo=gmail&logoColor=white" alt="Badge Email">
  </a>
  <a href="https://www.linkedin.com/in/ali-bouziane-063a74225/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Ali%20Bouziane-blue?style=flat-square&logo=linkedin&logoColor=white" alt="Badge LinkedIn">
  </a>
</div>

---

## 🧑‍💻 À propos de moi

- 🎓 Étudiant en **dernière année du cycle d’ingénieur en Génie Informatique** à l’Université Privée de Fès  
- 🔍 À la recherche d’un **stage de fin d’études (PFE)** de 6 mois à partir de **février 2026**  
- 🛡️ Passionné par la **cybersécurité**, le **développement logiciel** et l’**intelligence artificielle**  
- 🏆 Lauréat du **1er Prix – Compétition d’innovation XR 2025**  
- 💼 Expériences significatives en **développement Web** et **ingénierie logicielle** (BTP & aéronautique)

---

## 🛠 Compétences Techniques

### 💻 Langages & Web
- **Langages :** Python, Java (Certifié SE 17), C, PHP, JavaScript, HTML5, CSS3  
- **Frameworks :** Spring Boot, Spring Security, JEE, React, Laravel, Express.js, Vue.js  

### 🛡️ Cybersécurité & DevOps
- **Outils :** Splunk, ELK Stack, Wazuh, Wireshark, Burp Suite, Nmap, Sqlmap  
- **Compétences :** Pentesting (Web & Active Directory), Cryptographie, Sécurité réseau, Analyse de logs VPN  
- **Infrastructure :** Linux, Windows Server, Docker, AWS, Nginx  

### 📊 Data & IA
- **Big Data :** Apache Hadoop, HDFS, Spark, Hive, Sqoop  
- **Intelligence Artificielle :** LLMs, RAG, Agents IA, Machine Learning (K-Means, DBSCAN)  
- **Bases de données :** MySQL, SQL Server, MongoDB  

---

## 🎓 Certifications Clés

- 📜 **(ISC)²** – Certified in Cybersecurity (CC)  
- 📜 **Oracle** – Certified Professional: Java SE 17 Developer  
- 📜 **Cisco** – CCNA & Junior Cybersecurity Analyst  
- 📜 **IBM** – Artificial Intelligence Analyst  
- 📜 **Coursera** – Business Intelligence Expert  

---

## 🚀 Projets Phares

### 🥽 Solution de Streaming Immersif XR – *1er Prix 2025*
- Développement d’une solution de streaming immersif pour les matchs en réalité étendue utilisant **StereoKit** et **C#**

### 🕵️ Pentesting & Audit de Sécurité
- Audit complet d’une application web (**Cars-Enjoy**)  
  - Exploitation de **11 vulnérabilités critiques/élevées**  
  - RCE via File Upload, IDOR, Broken Access Control  
  - Rédaction d’un **rapport professionnel**
- Énumération et audit de sécurité **Active Directory**

### 🏦 Big Data – Secteur Bancaire
- Conception d’un **pipeline sécurisé** pour l’analyse et la protection des données bancaires avec **Hadoop & Hive**

---

## 🌐 Langues & Soft Skills

- **Langues :**  
  - Arabe : Maternelle  
  - Français : Courant / Avancé  
  - Anglais : Courant / Intermédiaire  

- **Soft Skills :**  
  Esprit d’équipe, rigueur, capacité d’analyse, sens du design graphique  

---

## 📞 Contact

- 📧 **Email :** [bouziane_ali@upf.ac.ma](mailto:bouziane_ali@upf.ac.ma)  
- 📱 **Téléphone :** 07 70 07 57 01  
- 💼 **LinkedIn :** [ali-bouziane-063a74225](https://www.linkedin.com/in/ali-bouziane-063a74225/)
